...
message.react("??")
  ...
  client.on('message', async (message) => {
    if(message.content.startsWith("&تقديم")) {
      await message.channel.send("** اسمك الحقيقى بالكامل**").then(e => {
      ...
      let chaLan = message.channel.awaitMessages(filter, { max: 1, time: 400000, errors: ['time'] })
        .then(collected => {
          ...
          e.delete();
          message.channel.send('**عندك كام سنة**').then(m => {
          let chaMd = message.channel.awaitMessages(filter, { max: 1, time: 400000, errors: ['time'] })
            .then(co => {
              md = co.first().content
              co.first().delete()
              m.delete();
              message.channel.send('**هل راح تساعدنا في الكريديت والفعاليات ؟**').then(ms => {
...
let banChannel = msg.guild.channels.find("name","▪『bots』");
...
let channel = member.guild.channels.find('name', '▫『welcome』');
  if (!channel) return; 
client.channels.get("518164471030611978").send(`مرحبا بك`) 
});
...
if (message.content === '&invite') {
      message.author.send('  https://discordapp.com/oauth2/authorize?client_id=524246659589210112&permissions=8&scope=bot  |  تفضل ربط البوت     ').catch(e => console.log(e.stack));
...
client.login("NTI1MDQ4NjQ1MjQ3MzAzNzEx.DvzoIw.gmQBcUa7RWjE_6qt6Keyy21u79g");//لاتغير شي